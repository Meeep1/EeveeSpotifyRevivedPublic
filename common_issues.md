...
EeveeSpotify currently supports Spotify version **9.1.28**.

If you are jailbroken, install the latest `.deb` from the [releases page](https://github.com/Meeep1/EeveeSpotifyRevivedPublic/releases), along with the recommended Spotify version. After installation, open the EeveeSpotify settings (accessible from your Spotify profile settings) and reset data so it can properly patch Premium-related behavior.
...
New versions are released when compatible Spotify updates become available. Check the [releases page](https://github.com/Meeep1/EeveeSpotifyRevivedPublic/releases) for the latest builds, or join the [Telegram channel](https://t.me/eeveespotifyrevived) for updates.
...
EeveeSpotify does not accept free feature requests. If you need something, feel free to implement it yourself, or submit a pull request if you think others may find it useful.

Note that many Premium features are server-sided and will never work without a Premium subscription (for example: very high quality audio and most download-related behavior).

# Troubleshooting & Issues

## Before opening an issue

Please include all of the following when reporting a bug:
- Spotify version
- EeveeSpotify version
- iOS version
- Install method (**TrollStore / AltStore / Sideloadly / Jailbreak / other**)
- Login method (**username-password / Google / Apple / Facebook / other**)
...
## Something Went Wrong / Cannot Sign In
...
5. Try signing in with **username/password** instead of a federated login method

## Logout While Playing

Some users have reported being logged out after a few minutes or after a few songs.

Before opening a new issue:
1. Search existing issues for logout reports
2. Mention exactly **when** it happens (for example, after 1–2 songs, after backgrounding, after 5–10 minutes)
3. Include your install method and login method
...
**Region issue**: You can only use Spotify abroad for 14 days. Possible solution:
...
This issue is often related to your account region. Do not enable **Overwrite Configuration** unless you've also tried the region fix. If your region is already correct, it can still help to temporarily switch regions and sign in again.

## Install Issues

If the app does not install:
1. Verify that the IPA is valid and contains `Payload/*.app/Info.plist`
2. Try a different installer
3. Make sure your signing method supports the entitlements you need

## Crashes on Newer Versions

If the app crashes on startup, your Spotify version may not be supported yet.

When reporting startup crashes, always include:
- Exact Spotify version
- Exact EeveeSpotify version
- Install method
- iOS version
...
Downloading is not, and will never be, implemented in EeveeSpotify.
...
Any issues regarding downloading of any kind may be closed.
