...
class SPTDataLoaderServiceHook: ClassHook<NSObject>, SpotifySessionDelegate {
    ...
    func URLSession(
        ...
        do {
            ...
            var buffer: Data? = nil // Declare buffer here, make it optional

            // If we are modifying the endpoint, ensure we have the data
            if shouldModify(url) {
                buffer = URLSessionHelper.shared.obtainData(for: url)
                guard buffer != nil else { // Check if data was obtained
                    orig.URLSession(session, task: task, didCompleteWithError: error)
                    return
                }
            }
            
            // Now, use the buffer for modification logic
            if url.isPremiumPlanRow {
                guard let dataBuffer = buffer else { // Ensure buffer exists for this case
                    orig.URLSession(session, task: task, didCompleteWithError: error)
                    return
                }
                respondWithCustomData(
                    try getPremiumPlanRowData(
                        originalPremiumPlanRow: try PremiumPlanRow(serializedBytes: dataBuffer)
                    ),
                    task: task,
                    session: session
                )
            ...
            if url.isCustomize {
                ...
                var customizeMessage = try CustomizeMessage(serializedBytes: dataBuffer)
            ...
            // If no specific modification was applied, call the original handler
            orig.URLSession(session, task: task, didCompleteWithError: error)
        }
        catch {
            orig.URLSession(session, task: task, didCompleteWithError: error)
